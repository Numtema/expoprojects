  ...
  return (
    <div className="w-full max-w-4xl mx-auto p-4 sm:p-6">
      <form onSubmit={handleSubmit} className="relative">
        <div className="bg-stone-200 p-1.5 sm:p-3 rounded-[1.5rem] sm:rounded-[3rem] shadow-2xl">
          <textarea
            value={idea}
            onChange={(e) => setIdea(e.target.value)}
            placeholder="app minimall todo list"
            className={cn(
              "w-full min-h-[160px] sm:min-h-[220px] p-6 sm:p-10 rounded-[1.2rem] sm:rounded-[2.5rem] bg-white border-none shadow-inner transition-all duration-300",
              "focus:ring-0 resize-none text-lg sm:text-2xl font-sans leading-relaxed text-stone-800",
        ...
        <div className="absolute bottom-4 sm:bottom-10 right-4 sm:right-10 flex items-center gap-2 sm:gap-4">
          <motion.button
            ...
            disabled={isLoading}
            className={cn(
              "p-2 sm:p-3 rounded-lg sm:rounded-xl transition-all duration-300 shadow-sm",
              isRecording 
                ? "bg-rose-500 text-white animate-pulse" 
                : "bg-stone-100 text-stone-500 hover:bg-stone-200"
            )}
          >
            {isRecording ? <Square className="w-3.5 h-3.5 sm:w-5 sm:h-5" /> : <Mic className="w-3.5 h-3.5 sm:w-5 sm:h-5" />}
          </motion.button>
          ...
          <motion.button
            ...
            className={cn(
              "px-4 sm:px-6 py-2 sm:py-3 rounded-lg sm:rounded-xl font-medium flex items-center gap-2 transition-all duration-300 text-xs sm:text-base",
              isLoading 
